...
The assignment requires developing a software architecture for the control of a robot in the environment. The {\bfseries{\mbox{\hyperlink{namespacego__to__point}{go\+\_\+to\+\_\+point}}}} node must be modelled as a R\+OS action server. During the simulation, the user can press \char`\"{}1\char`\"{} to give a random goal to the robot, and then press \char`\"{}0\char`\"{} to stop the robot.\hypertarget{md__r_e_a_d_m_e_autotoc_md2}{}\doxysubsection{Launch the simulation}\label{md__r_e_a_d_m_e_autotoc_md2}
\hypertarget{md__r_e_a_d_m_e_autotoc_md3}{}\doxysubsubsection{Simulation with Gazebo}\label{md__r_e_a_d_m_e_autotoc_md3}
...
\end{DoxyCode}
\hypertarget{md__r_e_a_d_m_e_autotoc_md4}{}\doxysubsubsection{Simulation with V\+Rep}\label{md__r_e_a_d_m_e_autotoc_md4}
...
\end{DoxyCode}
\hypertarget{md__r_e_a_d_m_e_autotoc_md5}{}\doxysection{Nodes and program description}\label{md__r_e_a_d_m_e_autotoc_md5}
...
\item The second one, {\bfseries{\mbox{\hyperlink{user__interface_8py}{user\+\_\+interface.\+py}}}}, is a client used to read the user command and to send it to the {\bfseries{state\+\_\+machine}} node. This node sends the new command through the service message {\itshape Command} and has an action client, which is connected to {\bfseries{\mbox{\hyperlink{namespacego__to__point}{go\+\_\+to\+\_\+point}}}}. If the user presses \char`\"{}1\char`\"{}, the node sends a \char`\"{}\+\_\+start\+\_\+\char`\"{} message to the {\itshape \mbox{\hyperlink{namespaceuser__interface}{user\+\_\+interface}}} server; if the user types \char`\"{}0\char`\"{}, the node cancels the goal, using the action client, and sends a \char`\"{}\+\_\+stop\+\_\+\char`\"{} message to the {\itshape \mbox{\hyperlink{namespaceuser__interface}{user\+\_\+interface}}} server.
...
\begin{DoxyItemize}
\item The first one, {\bfseries{gazebo\+Sim.\+launch}}, used to launch the Gazebo environment and to run the four nodes.
\item The second one, {\bfseries{vrep\+Sim.\+launch}}, used to launch only the four nodes. 
\end{DoxyItemize}